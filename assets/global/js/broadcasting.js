...
function marketChangeHtml(data) {
    $.each(data.marketData, function (i, marketData) {
        ...
        $(`body`).find(`.market-price-${marketData.id}`).text(getAmount(marketData.price,8)).removeClass('up down').addClass(htmlClass.price_change);
        ...
        $(`body`).find(`.market-last-price-${marketData.id}`).text(getAmount(marketData.last_price, 8));
