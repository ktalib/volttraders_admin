...
class Cryptocompare extends CurrencyDataProvider
{
    ...
    public function updateCryptoPrice()
    {
        ...
        foreach ($data->RAW ?? [] as $item) {
            ...
            if (!$cryptoCurrency || !$itemData || !$marketData) {
                continue;
            }

            $cryptoCurrency->rate = $itemData->PRICE;
            if ($cryptoCurrency->last_midnight_update != today()) {
                $cryptoCurrency->midnight_rate        = $itemData->PRICE;
                $cryptoCurrency->last_midnight_update = today();
            }
            $cryptoCurrency->save();
    ...
    public function import($parameters, $type)
    {
        ...
        if ($type == Status::FIAT_CURRENCY || $importCount <= 0) {
            return $importCount;
        }
        ...
        foreach ($currencies as $currency) {
            ...
            if (MarketData::where('pair_id', 0)->where('currency_id', $currency->id)->exists()) {
                continue;
            }
