...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $totalTrade       = Trade::where('trader_id', $user->id)->count();
        $totalDeposit     = Deposit::where('user_id', $user->id)->successful()->count();
        $totalWithdrawals = Withdrawal::where('user_id', $user->id)->approved()->count();
        $currencies       = Currency::active()->get();
        $countries        = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        return view('admin.users.detail', compact('pageTitle', 'user', 'totalTrade', 'totalOrder', 'totalDeposit', 'totalWithdrawals', 'currencies', 'countries'));
    }
